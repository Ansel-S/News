...
step_yt() {
    ...
    if [ "$count" -gt 0 ]; then
        ok "youtube.db: $count videos ($yt_count with subtitles)"
    else
...
step_render() {
    ...
    # Uses explicit column names (not positional VALUES) so this keeps working
    # if a table gains columns later — a bare VALUES(...) breaks the instant
